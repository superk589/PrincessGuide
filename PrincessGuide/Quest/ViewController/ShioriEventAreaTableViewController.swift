//
//  ShioriEventAreaTableViewController.swift
//  PrincessGuide
//
...
class ShioriEventAreaTableViewController: UITableViewController, DataChecking {
    
    private var events = [ShioriEvent]()
    ...
    private func loadData() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            Master.shared.getShioriEvents { (events) in
                // preload
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let event = events[indexPath.row]
        let vc = QuestEnemyTableViewController(shioriEvent: event)
